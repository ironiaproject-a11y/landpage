...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const heroSubhead = heroSubheadRef.current;
    const heroBtn     = heroBtnRef.current;
    
    if (!video || !container || !heroContent || !originText || !smileText || !heroSubhead || !heroBtn) return;
    ...
    const ctx = gsap.context(() => {
      ...
      const initMasterTimeline = () => {
        ...
        const buildTimelines = (duration: number) => {
          ...
          gsap.set(heroSubhead, { opacity: 0, y: 15 });
          ...
          mainTl.fromTo(heroSubhead,
            { opacity: 0, y: 15 },
            { opacity: 1, y: 0, duration: duration * 0.2, ease: "power2.out" }, duration * 0.82);

          mainTl.fromTo(heroBtn,
            { opacity: 0, y: 30, scale: 0.9 },
            { opacity: 1, y: 0, scale: 1, duration: duration * 0.2, ease: "back.out(1.7)" }, duration * 0.88);
        ...
        >
          ...
          <div ref={heroContentRef} className="hero-content-split" style={{ pointerEvents: "auto", position: 'relative', height: '100%', zIndex: 2 }}>
            ...
            <div className="hero-text-group">
              ...
              <p ref={heroSubheadRef} className="hero-sub-headline">Cuidado odontológico de excelência em São Paulo.</p>
            </div>
